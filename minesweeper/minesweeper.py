...
class Sentence():
    ...
    and a count of the number of those cells which are mines but not found
    """
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # based on the cell and count commit to AI's knowledge
        cells = set()
        count_cpy = deepcopy(count)
        close_cells = self.return_close_cell(cell)  # return neighbor cell
        for cl in close_cells:
            if cl in self.mines:
                count_cpy -= 1
            if cl not in self.mines | self.safes:
                cells.add(cl)  # only add cell that not known
    ...
    def return_close_cell(self, cell):
        close_cells = set()
    ...
    def check_knowledge(self):
        knowledge_cpy = deepcopy(self.knowledge)
